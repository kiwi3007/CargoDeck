...
func (h *Handler) GetSavePaths(w http.ResponseWriter, r *http.Request) {
	...
	if agentHome != "" && !filepath.IsAbs(agentHome) {
		http.Error(w, "home must be an absolute path", http.StatusBadRequest)
		return
	}
	if agentHome == "" {
	...
	// If the user specified custom save paths for this device, use them exclusively.
	// Don't mix with manifest/fallback — the user told us exactly where saves are.
	var customPaths []string
	if agentID != "" && game != nil {
		customPaths, _ = h.repo.GetAgentSavePath(game.ID, agentID)
	}
	if len(customPaths) == 0 && game != nil && game.SavePath != "" {
		customPaths = []string{game.SavePath}
	}
	if len(customPaths) > 0 {
		log.Printf("[Saves] GetSavePaths %q agent=%s: using custom paths %v", title, agentID, customPaths)
		jsonOK(w, map[string]any{"paths": customPaths, "gameId": gameID})
		return
	}
...
func (h *Handler) DeleteSaveSnapshot(w http.ResponseWriter, r *http.Request) {
	...
	// Security: ensure path is strictly under saves dir (suffix-collision fix)
	savesBase := h.saveBaseDir(gameID)
	cleanDir := filepath.Clean(snapshotDir)
	cleanBase := filepath.Clean(savesBase) + string(filepath.Separator)
	if !strings.HasPrefix(cleanDir, cleanBase) {
		http.Error(w, "invalid path", http.StatusBadRequest)
		return
	}

	// Also resolve any symlinks in the parent to block symlink-based traversal.
	if resolved, err := filepath.EvalSymlinks(filepath.Dir(snapshotDir)); err == nil {
		if !strings.HasPrefix(resolved+string(filepath.Separator), cleanBase) {
			http.Error(w, "invalid path", http.StatusBadRequest)
			return
		}
	}
...
func extractTarGz(r io.Reader, destDir string) error {
	...
	for {
		...
		case tar.TypeSymlink, tar.TypeLink:
			continue // reject — could point outside destination
		case tar.TypeDir:
...
func (h *Handler) GetSavePathsInfo(w http.ResponseWriter, r *http.Request) {
	...
	for _, paths := range agentPaths {
		for _, p := range paths {
			if p != "" && !seenCustom[p] {
				seenCustom[p] = true
				customPathsList = append(customPathsList, p)
			}
		}
	}
	// Include legacy global path as a custom path
	if game.SavePath != "" && !seenCustom[game.SavePath] {
		customPathsList = append(customPathsList, game.SavePath)
	}

	// Merged list: custom paths first, then detected base paths, deduped
	merged := dedupePaths(append(customPathsList, basePaths...))
...
func (h *Handler) SetSavePath(w http.ResponseWriter, r *http.Request) {
	...
	var body struct {
		SavePath  string   `json:"savePath"`  // legacy single-path (still accepted)
		SavePaths []string `json:"savePaths"` // preferred: multiple paths
		AgentID   string   `json:"agentId"`
	}
	...
	// Normalise: prefer savePaths array; fall back to single savePath
	paths := body.SavePaths
	if len(paths) == 0 && body.SavePath != "" {
		paths = []string{body.SavePath}
	}

	if body.AgentID != "" {
		// Per-device override
		if err := h.repo.SetAgentSavePath(gameID, body.AgentID, paths); err != nil {
