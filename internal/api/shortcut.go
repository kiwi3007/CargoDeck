...
func addSteamShortcut(entry shortcutEntry) (uint32, error) {
	...
	// Replace if AppName or Exe already exists
	replaced := false
...
func parseShortcutsVDF(data []byte) []shortcutEntry {
	...
	// Expect: \x00 "shortcuts\x00" then the entries
	...
	for pos < len(data) {
		...
		pos++ // consume map type byte
		pos = skipCString(data, pos) // skip entry index ("0", "1", …)
		...
		for pos < len(data) && data[pos] != 0x08 {
			...
			case 0x01: // null-terminated string
			...
			case 0x00: // nested map — skip entire sub-map
				pos = skipMap(data, pos)
			}
		}
		if pos < len(data) && data[pos] == 0x08 {
			pos++ // consume end-of-entry marker
		}
...
// skipMap skips all key-value pairs in a nested map until \x08, returning pos after the \x08.
func skipMap(data []byte, pos int) int {
	for pos < len(data) && data[pos] != 0x08 {
		typeByte := data[pos]
		pos++
		pos = skipCString(data, pos) // key
		switch typeByte {
		case 0x01:
...
func buildShortcutsVDF(entries []shortcutEntry) []byte {
	...
	for i, e := range entries {
		...
		buf.WriteByte(0x08) // end of tags map

		buf.WriteByte(0x08) // end of entry map
	}

	buf.WriteByte(0x08) // end of shortcuts map
	buf.WriteByte(0x08) // end of root
	return buf.Bytes()
}
