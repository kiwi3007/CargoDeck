...
func (c *Client) restoreLatestSave(gameID int, title string) {
	...
	if game == nil {
		log.Printf("[Saves] Restore: game %q not watched — skipping", title)
		return
	}

	// For glob patterns with no existing matches (fresh install), restore into the
	// glob parent dir so the archive's directory structure is preserved naturally.
	restoreDirs := game.saveDirs
	if len(restoreDirs) == 0 {
		for _, raw := range game.rawPaths {
			if isGlobPath(raw) {
				parent := filepath.Dir(raw)
				restoreDirs = append(restoreDirs, parent)
				log.Printf("[Saves] Restore: no glob matches for %q, using parent %s", title, parent)
			}
		}
	}
	if len(restoreDirs) == 0 {
		log.Printf("[Saves] Restore: no save dirs for %q — skipping", title)
		return
	}
	...
	restored := 0
	for _, saveDir := range restoreDirs {
		...
		// 1. Exact basename match — the archive was made from a dir with the same name.
		//    This covers cross-device glob restores where the pattern is e.g. /game/*/saves:
		//    both devices archive "saves/" so the basename matches regardless of the random
		//    middle component.
		if srcDir := filepath.Join(tmpDir, base); dirExists(srcDir) {
			copyDir(srcDir, saveDir)
			restored++
			log.Printf("[Saves] Restored %q → %s (basename match)", base, saveDir)
			continue
		}

		// 2. No basename match — the random directory name differs (glob path ending in *).
		//    Copy the archive's top-level dir contents directly into saveDir so the local
		//    random name is preserved rather than overwritten with the remote name.
		if len(topDirs) > 0 {
			srcDir := filepath.Join(tmpDir, topDirs[0])
			copyDir(srcDir, saveDir)
			restored++
			log.Printf("[Saves] Restored %q → %s (random dir mismatch, merged into local name)", topDirs[0], saveDir)
			continue
		}
...
type watchedSaveGame struct {
	...
	saveDirs  []string // resolved exact directories (globs expanded)
	rawPaths  []string // original paths as returned by server (may contain * globs)
}

// isGlobPath reports whether path contains a glob wildcard.
func isGlobPath(p string) bool { return strings.ContainsAny(p, "*?[") }

// resolveGlobDirs expands a glob pattern to matching directories.
// Returns the parent dir (watched for newly created subdirs) and the matched dirs.
func resolveGlobDirs(pattern string) (parent string, dirs []string) {
...
// currentSaveDirs re-resolves globs in rawPaths and returns all current directories.
// Use at upload time so newly created subdirs are always included.
func (g *watchedSaveGame) currentSaveDirs() []string {
	var dirs []string
	for _, p := range g.rawPaths {
		if isGlobPath(p) {
			_, resolved := resolveGlobDirs(p)
...
func (sw *SaveWatcher) UpdateGames(titles []string, scriptDirs map[string]string) {
	...
	for _, title := range titles {
		...
		// Expand glob patterns into resolved dirs; also collect glob parent dirs to watch.
		var resolvedDirs []string
		for _, p := range saveDirs {
			if isGlobPath(p) {
				parent, matched := resolveGlobDirs(p)
				// Watch the parent so we detect newly created subdirs.
				if err := sw.watcher.Add(parent); err == nil {
					log.Printf("[Saves] Watching glob parent %s for %q", parent, title)
				}
				resolvedDirs = append(resolvedDirs, matched...)
			} else {
				resolvedDirs = append(resolvedDirs, p)
			}
		}

		existing := sw.games[sn]
		game := &watchedSaveGame{title: title, gameID: gameID, scriptDir: scriptDir, saveDirs: resolvedDirs, rawPaths: saveDirs}
		sw.games[sn] = game

		// Remove watches for directories that are no longer in the list
		if existing != nil {
			for _, old := range existing.saveDirs {
				if !containsStr(resolvedDirs, old) {
		...
		// Watch each resolved save directory recursively.
		sw.mu.Unlock()
...
func (sw *SaveWatcher) handleEvent(event fsnotify.Event) {
	...
	if event.Op&fsnotify.Create != 0 {
		if fi, err := os.Stat(event.Name); err == nil && fi.IsDir() {
			...
			// Check if this new dir matches any glob pattern and register it.
			sw.mu.Lock()
			for _, game := range sw.games {
				for _, raw := range game.rawPaths {
					if !isGlobPath(raw) {
						continue
					}
					matched, _ := filepath.Match(raw, event.Name)
					if matched && !containsStr(game.saveDirs, event.Name) {
						game.saveDirs = append(game.saveDirs, event.Name)
						log.Printf("[Saves] New glob match for %q: %s", game.title, event.Name)
						// Immediately restore the latest server snapshot into this freshly-created
						// dir. This prevents the game's initial-state save from overwriting real
						// progress stored on the server (e.g. from another device). restoreLatestSave
						// acquires sw.mu so it must run in a goroutine.
						if game.gameID > 0 {
							gameID := game.gameID
							title := game.title
							go sw.client.restoreLatestSave(gameID, title)
						}
					}
				}
			}
...
func (c *Client) uploadSaves(g *watchedSaveGame) {
	// Re-resolve globs at upload time to pick up any dirs created since watch setup.
	dirs := g.currentSaveDirs()
	if len(dirs) == 0 {
		log.Printf("[Saves] Upload skipped for %q: no save dirs found", g.title)
		return
	}

	log.Printf("[Saves] Building snapshot for %q: %v", g.title, dirs)
